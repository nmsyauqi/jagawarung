...
class _TutupShiftPageState extends State<TutupShiftPage> {
  ...
  Future<void> _tutupShift() async {
    ...
    final nominal = int.tryParse(_raw()) ?? 0;
  ...
  Widget build(BuildContext context) {
    ...
    if (!shiftProvider.isShiftActive) return const Scaffold();
    
    final shift = shiftProvider.activeShift!;
    final totalMasuk = shiftProvider.totalUangMasuk;
    final jumlahTx = shiftProvider.listTransaksi.length;
    final saldoSeharusnya = shift.saldoAwal + totalMasuk;
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _baris('Jumlah Transaksi', '$jumlahTx transaksi'),
                  const SizedBox(height: 10),
                  _baris('Total Uang Masuk', Fmt.uang(totalMasuk.toDouble())),
                  const Divider(height: 24),
                  _baris('Saldo Seharusnya', Fmt.uang(saldoSeharusnya.toDouble()), tebal: true),
                ],
              ),
            ),
            const SizedBox(height: 16),

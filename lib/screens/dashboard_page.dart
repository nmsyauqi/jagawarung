...
class _DashboardPageState extends State<DashboardPage> {
  ...
  void _keTutupShift() {
    Navigator.of(context).push(MaterialPageRoute(builder: (_) => const TutupShiftPage())).then((_) {
      if (!context.read<ShiftProvider>().isShiftActive) {
            ...
            Row(
              children: [
                ...
                Expanded(child: ElevatedButton(
                  ...
                  onPressed: () { Navigator.pop(ctx); /* AppState.logout(); */ Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (_) => const LoginPage())); },
  ...
  Widget build(BuildContext context) {
    ...
    final s = targetProvider.shiftAktif!;
    // Mock pegawai sampai AuthProvider siap
    final p_isPemilik = false;
    final p_inisial = 'P';
    final p_nama = 'Pekerja Dummy';
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          if (p_isPemilik)
            _appBarBtn(Icons.bar_chart_rounded, () => Navigator.push(context, MaterialPageRoute(builder: (_) => const RiwayatPage()))),
        ...
        children: [
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Expanded(child: _statItem('Modal Awal', Fmt.uang(s.saldoAwal), Icons.account_balance_wallet_outlined, AppTheme.textBody)),
                    Container(width: 1, height: 50, color: AppTheme.borderLight),
                    Expanded(child: _statItem('Uang Masuk', Fmt.uang(targetProvider.totalUangMasuk), Icons.south_west_rounded, AppTheme.accent)),
                  ],
                ),
                ...
                Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Text(Fmt.uang(s.saldoSeharusnya), style: GoogleFonts.plusJakartaSans(fontSize: 18, fontWeight: FontWeight.w800, color: AppTheme.accent)),
                    ],
                  ),
                ),
              ],
          ...
          Row(
            children: [
              ...
              Container(
                ...
                child: Text('${s.transaksi.length}', style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w700, color: AppTheme.accent)),
              ),
            ],
          ),
          const SizedBox(height: 12),
          ...
          if (s.transaksi.isEmpty)
            ...
            Container(
              ...
              child: Column(
                children: [
                  for (int i = s.transaksi.length - 1; i >= 0; i--) ...[
                    _txItem(s.transaksi[i]),
  ...
  Widget _txItem(Transaksi tx) {
    final ada = tx.catatan != null && tx.catatan!.isNotEmpty;
    return Padding(
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(ada ? tx.catatan! : 'Uang Masuk', style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.w500, color: AppTheme.textDark), maxLines: 1, overflow: TextOverflow.ellipsis),
                const SizedBox(height: 2),
                Text(Fmt.jam(tx.waktu), style: GoogleFonts.inter(fontSize: 12, color: AppTheme.textMuted)),
              ],
            ),
          ),
          Text('+${Fmt.uang(tx.nominal)}', style: GoogleFonts.plusJakartaSans(fontSize: 15, fontWeight: FontWeight.w700, color: AppTheme.accent)),
        ],
      ),
    );
  }

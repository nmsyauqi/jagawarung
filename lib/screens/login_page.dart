...
class _MasukWarungPageState extends State<MasukWarungPage> {
  ...
  void _prosesMasuk() async {
    ...
    ); // Animasi loading mock

    if (!mounted) return;

    // MOCKUP LOGIC:
    // sandi owner123 -> Dashboard Owner, selain itu -> Pegawai
    if (sandi == 'owner123') {
      ...
      // Logic Pegawai
      final shiftAktif = context.read<ShiftProvider>().isShiftActive;
      if (!shiftAktif) {
        Navigator.pushAndRemoveUntil(
          context,
          MaterialPageRoute(builder: (_) => const BukaShiftPage()),
          (route) => false,
        );
      } else {
        Navigator.pushAndRemoveUntil(
